...
**Motion** is D-ID **Agents Streams** (WebRTC) via `@d-id/client-sdk`. The Worker mints a short-lived `client_key` (`POST /stream`) for the allowed Surge origins. Ask Sara connects on **Send / Play** (parallel with `POST /ask` + ara) — it does **not** pre-warm `connect()` on mount, so an idle tab does not hold a Lite session slot. `agentManager.speak({ type: 'text', input })` fires as soon as the Grok reply exists (do not wait for ara or D-ID START). Leave / unmount / `pagehide` disconnect immediately; a short idle after speak also releases the peer. The stream video is muted so ara is the only voice. Do not call `agentManager.chat()` — Grok stays the brain (`POST /ask`).
