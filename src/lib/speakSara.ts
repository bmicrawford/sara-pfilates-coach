...
/** Client waits this long for D-ID video after POST /talk returns a talkId. */
export const SARA_TALK_POLL_MS = 150_000
const TALK_START_MS = 45_000
const TALK_POLL_EVERY_MS = 2_000
...
function isTalkDone(status?: string, reason?: string) {
  return status === 'error' || status === 'rejected' || reason === 'missing_did_key' || reason === 'missing_id'
}
...
export async function requestSaraTalk(text: string): Promise<string | null> {
  ...
  const startTimer = window.setTimeout(() => startCtrl.abort(), TALK_START_MS)
  ...
  try {
    ...
    if (!data.talkId || isTalkDone(undefined, data.reason)) return null
    talkId = data.talkId
  } catch {
    return null
  } finally {
  ...
  const deadline = Date.now() + SARA_TALK_POLL_MS
  while (Date.now() < deadline) {
    await wait(TALK_POLL_EVERY_MS)
    const pollCtrl = new AbortController()
    const pollTimer = window.setTimeout(() => pollCtrl.abort(), 12_000)
    try {
      const poll = await fetch(`${apiBase()}/talk?id=${encodeURIComponent(talkId)}`, {
      ...
      if (isTalkDone(body.status, body.reason)) return null
    } catch {
      /* keep polling until the budget; D-ID often needs ~110s */
    } finally {
