...
- **D-ID talking-head is Agents Streams (WebRTC), not Talks mp4.** Worker `POST /stream` mints a short-lived Agents SDK `client_key` (`ttl_seconds` ~10 min) for `https://sara-pfilates.surge.sh` and `https://sara-pfilates-coach.surge.sh` (plus local Vite). Browser uses `@d-id/client-sdk` `createAgentManager` + `speak({ type: 'text', input })` with the Grok reply. Do **not** `connect()` on Ask Sara mount (Lite concurrent-session cap). Mint + WebRTC on Send / Play only; keep the session without gating on decoded frames; fire `speak()` as soon as Grok text exists. Heard voice is D-ID stream audio when speak START (or a real speak result) arrives in a short budget — unmute the WebRTC element and do **not** also play ara. If connect/speak fails, is session-capped, or is slow past that budget, fall back to xAI `ara` via `POST /speak` + still (or muted video if frames exist). Release on leave / unmount / `pagehide`. Do not call `chat()` (Grok is the brain via `POST /ask`). Never block on D-ID clip generation. Missing `DID_API_KEY` or `DID_AGENT_ID` → honest null credentials, ara still works. Do not restore the SVG mouth overlay. Do not put `DID_API_KEY` in the Vite bundle. CORS must keep both Surge hosts. Default still: `https://sara-pfilates.surge.sh/avatar/sara-default.png`. `403 Forbidden` / `Max user sessions reached` (or zero D-ID credits) is a dashboard credit/session-cap issue: do not retry connect/speak, do not recreate the agent, keep still + ara.
